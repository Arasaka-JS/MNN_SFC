...
ErrorCode ConvolutionPackWinograd::onResize(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
    ...
    mMainFunction.second = [=](int tId, const uint8_t* inputOrigin, uint8_t* dstOrigin) {
        ...
        // float dst_sfc[8*10]={0};
        
        // mSourceUnrollTransform(src,dst_wino,8,10,1,1);
        // std::cout<<"source trans test"<<std::endl;
        ...
        // //source trans test01
        // int ic4_test = 1;int batch_test = 1;int epack_test=24;int ih_test = 8;int iw_test=8*epack_test;int pack_test = 8;int srcUnit_test=8;int midUnit_test=10;int bytes_test=4;

        // uint8_t source_origin_test[ic4_test*batch_test*ih_test*iw_test*pack_test*bytes_test];

        // auto source_origin_test_ptr = source_origin_test;
        // auto source_origin_test_float_ptr = (float*)source_origin_test;

        // for(int i=0;i<ic4_test*batch_test*ih_test*iw_test*pack_test;i++)
        // {
        //      *(source_origin_test_float_ptr+i)=1+0.0001*i;
        //     //source_origin_test_float_ptr[i]=1;
        // }

        // for(int i=0;i<ic4_test*batch_test*ih_test*iw_test*pack_test;i++)
        // {
        //     // source_origin_test[i]=1+0.0001*i;
        //     std::cout<<source_origin_test_float_ptr[i]<<",";
        // }
        // std::cout<<std::endl;

        
        // uint8_t mTempBuffer_test[ic4_test*srcUnit_test*midUnit_test*epack_test*pack_test*bytes_test];
        // auto    mTempBuffer_ptr = mTempBuffer_test;

        // for(int si_test=0;si_test<epack_test;si_test++)
        // {

        //     auto srcStart_test = source_origin_test_ptr + si_test*srcUnit_test*pack_test*bytes_test;

            // for(int i=0;i<8;i++)
            // {   for(int j=0;j<8;j++)
            //     {
            //         std::cout<<((float*)srcStart_test)[j+i*iw_test*pack_test]<<",";
            //     }
            // }
            // std::cout<<std::endl;

        //     auto dstStart_test = mTempBuffer_ptr + si_test*pack_test*bytes_test;

        //     for(int z =0;z<ic4_test;z++)
        //     {                            

        //         auto srcZ = srcStart_test + z* iw_test * ih_test * batch_test * pack_test * bytes_test;
        //         auto dstZ = dstStart_test + z * srcUnit_test*midUnit_test*epack_test*pack_test*bytes_test;
        //         mSourceUnrollTransform((const float*)srcZ,(float*)dstZ, iw_test*pack_test,  epack_test*pack_test,   pack_test, srcUnit_test*epack_test*pack_test);
        //     }
        // }

        // auto mTempBuffer_float_ptr = (float*)mTempBuffer_ptr;
        // for(int i=0;i<ic4_test*srcUnit_test*midUnit_test*epack_test*pack_test;i++)
        // {
        //     std::cout<<mTempBuffer_float_ptr[i]<<",";
        //     if(i%(epack_test*pack_test)==0){std::cout<<std::endl;}
        // }
        // std::cout<<std::endl;
        // return ;

        //source trans test02
        // uint8_t mTempBuffer_test02[ic4_test*srcUnit_test*pack_test*epack_test*bytes_test];
        // auto mTempBuffer_test02_ptr = mTempBuffer_test02;
        // for(int i=0;i<8;i++)
        // {
        //     auto mTempBuffer_Start = mTempBuffer_test02_ptr + i * pack_test * epack_test * bytes;
        //     for(int j =0;j<pack_test * epack_test;j++)
        //     {
        //         auto mTempBuffer_Start_z = (float*)mTempBuffer_Start + j;
        //         *mTempBuffer_Start_z = i;
        //     }
        // }
        // for(int i=0;i<ic4_test*srcUnit_test*pack_test*epack_test;i++)
        // {
        //     std::cout<<(*(((float*)mTempBuffer_test02_ptr)+i))<<",";
        // }

        // uint8_t GemmMidBuffer[midUnit_test*ic4_test*pack_test*epack_test*bytes_test];
        // auto GemmMidBuffer_test02_ptr = GemmMidBuffer;
        // mSourceTransformPack((float*)mTempBuffer_test02_ptr,(float*)GemmMidBuffer_test02_ptr,epack_test*pack_test*ic4_test);

        // for(int i=0;i<midUnit_test*ic4_test*pack_test*epack_test;i++)
        // {
        //     std::cout<<(*(((float*)GemmMidBuffer_test02_ptr)+i))<<",";
        // }

        // //muilti test 03
        // int ic4_test = 1;int batch_test = 1;int epack_test=24;int ih_test = 8;int iw_test=8*epack_test;int pack_test = 8;int srcUnit_test=8;int midUnit_test=10;int bytes_test=4;
        // int coc4_test=8;int cic4_test=32;int unitci_test=1;int unitco_test=4;

        // float weight_test_03[coc4_test*cic4_test*unitci_test*unitco_test];
        // for(int i=0;i<coc4_test*cic4_test*unitci_test*unitco_test;i++)
        // {
        //     weight_test_03[i]=0.2;
        // }

        // float unitsGemmbuffer_test_03[ic4_test*pack_test*epack_test];
        // for(int i=0;i<ic4_test*pack_test*epack_test;i++)
        // {
        //     unitsGemmbuffer_test_03[i]=0; 
        // }

        // float dstFloat_test_03[ic4_test*coc4_test*cic4_test*unitci_test*unitco_test*pack_test*epack_test];
        // std::vector<size_t> parameters_test_03(6);
        // parameters_test_03[1] = input->channel();
        // parameters_test_03[2] = output->channel();
        // parameters_test_03[4] = 0;
        // parameters_test_03[5] = 0;
        // parameters_test_03[0] = 2;//没影响
        // parameters_test_03[3] = ePack * pack * bytes;

        // core->MNNPackedMatMul(dstFloat_test_03, unitsGemmbuffer_test_03, weight_test_03, parameters_test_03.data(), nullptr, nullptr, nullptr, nullptr);
